...
var locksCmd = &cobra.Command{
	Use:                   "locks",
	Short:                 "List all locks in the project",
	Long:                  "List all locks in the project",
	DisableFlagsInUseLine: true,
	Args:                  cobra.OnlyValidArgs,
	RunE: func(cmd *cobra.Command, args []string) error {
		conn := cmd.Context().Value("conn").(*client.Client)
		session := cmd.Context().Value("session").(*config.SessionConfig)
		resp, err := conn.CloudClient().GetProjectLocks(cmd.Context(), &cloudv1.GetProjectLocksRequest{
...
func init() {
	...
	RootProjectCommand.AddCommand(locksCmd)
}
